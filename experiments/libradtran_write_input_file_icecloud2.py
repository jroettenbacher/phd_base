...
Set up libRadtran like ecRad using input from the IFS and defining an ice cloud along track for a sensitivity study

Simulate an ice cloud along the case study period of RF17 (2022-04-11 10:30 - 12:30 UTC) with different IWC and
:math:`r_{eff, ice}` at 6.5 to 7.5 km altitude.
Generate output at every 100m in the vertical.
...
if __name__ == "__main__":
    ...
    iwc_user = [0.0001, 0.001, 0.005, 0.01]  # g/m³
    re_ice_user = [10, 20, 30, 40, 50, 60]  # mum

    # %% write input files for each timestep
    timestamps = pd.date_range("2022-04-11 10:30", "2022-04-11 12:30", freq=time_step)
    for timestamp in tqdm(timestamps, desc="Write input files"):
        ...
        zout = np.arange(0, 13.02, 0.1)
        zout = [f"{z:.3f}" for z in zout]
        zout_str = " ".join(zout)
        ...
        for iwc_u in iwc_user:
            for re_ice_u in re_ice_user:
                ...
                output_altitudes = np.flip(np.arange(0, 13.02, 0.1))
