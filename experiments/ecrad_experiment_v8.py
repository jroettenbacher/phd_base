...
.. figure:: figures/experiment_v8/HALO-AC3_20220411_HALO_RF17_ecrad_v8_iwc_along_track.png
    ...
    Retrieved ice water content from VarCloud retrieval.
...
.. figure:: figures/experiment_v8/HALO-AC3_20220411_HALO_RF17_ecrad_diff_re_ice_hist.png

    Histogramm of difference between |re-ice| IFS and |re-ice| VarCloud.

The differences in microphysical properties also translate to differences in optical properties of the cloud.
...
Due to a larger |re-ice| the ice cloud from the retrieval is less absorbing/reflecting than the IFS forecasted ice cloud.
...
if __name__ == "__main__":
    ...
    factor = ecrad_ds_v1.pressure_hl.diff(dim="half_level").to_numpy() / (9.80665 * ecrad_ds_v1.cloud_fraction.to_numpy())
    ...
    factor = ecrad_ds_v8.pressure_hl.diff(dim="half_level").to_numpy() / (9.80665 * ecrad_ds_v8.cloud_fraction.to_numpy())
    ...
    for t in tqdm(ecrad_plot.time, desc="New Z-Axis"):
        ...
        if "half_level" in tmp_plot.dims:
            ...
            tmp_plot = tmp_plot.assign_coords(level=ecrad_ds_v8["press_height_full"].sel(time=t, drop=True).to_numpy() / 1000)
