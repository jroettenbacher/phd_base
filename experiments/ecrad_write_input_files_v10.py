...
Use a processed IFS output file on a O1280 grid and generate one ecRad input
file for each time step.
Replace the water vapor mixing ratio with the (interpolated) ones from the
...
* init_time, initialization time of IFS run (00, 12, yesterday)
...
if __name__ == '__main__':
    ...
    # %% find the closest grid points along flight track
    ...
    # %% filter low clouds according to ECMWF's low cloud criterion (pressure higher than 0.8 * surface pressure)
    ...
    dropsonde_q = dropsonde_ds.q.interpolate_na(dim='alt',
                                                method='linear',
                                                fill_value='extrapolate')
    # replace sonde_id with launch time for easier selection
    dropsonde_q['launch_lat'] = dropsonde_ds['lat'].sel(alt=10000, drop=True)
    dropsonde_q['launch_lon'] = dropsonde_ds['lon'].sel(alt=10000, drop=True)
    dropsonde_q = (dropsonde_q
                   .assign_coords(sonde_id=dropsonde_q.launch_time)
                   .rename(sonde_id='time')
                   .sortby('time')
                   .set_index(latlon=('launch_lat', 'launch_lon')))
    ...
    for i in tqdm(range(0, idx)):
        ...
        # replace specific humidity
        t_sel = np.argmin([distance(latlon_sel, loc).km
                           for loc in
                           dropsonde_q['latlon'].to_numpy()])
        q_sel = (dropsonde_q
                 .isel(time=t_sel)
                 .drop_vars(['lat', 'lon', 'launch_time', 'interpolated_time', 'latlon']))
        q_new = q_sel.interp(alt=ds.press_height_full).drop_vars('alt')
        # interpolate nan values -> lowest value over ground
        q_new = q_new.interpolate_na(dim='level',
                                     method='linear',
                                     fill_value='extrapolate',
                                     limit=2)
        # replace values at altitudes above the dropsonde with the IFS data
        q_new = q_new.where(~np.isnan(q_new), ds.q)

        # plot interpolated and old data in debug mode
        if log.level < 20:
            _, ax = plt.subplots(layout='constrained')
            ((ds.q * 1e3)
             .assign_coords(level=ds.press_height_full)
             .plot(y='level',
                   label=f'IFS q ({pd.to_datetime(ds.time.to_numpy()):%Y-%m-%d %H:%M})\n'
                         f'Lat:{ds.lat.to_numpy()[0]:.2f}, Lon:{ds.lon.to_numpy()[0]:.2f}',
                   ax=ax))
            (q_sel * 1e3).plot(y='alt', label='Dropsonde q', ax=ax)
            ((q_new * 1e3)
             .assign_coords(level=ds.press_height_full)
             .plot(y='level', label='Interpolated q', ax=ax))
            ax.set(
                title=f'Dropsonde launch time: '
                      f'{pd.to_datetime(q_sel.time.to_numpy()):%Y-%m-%d %H:%M}',
            ...
            plt.savefig(f'debug/HALO-AC3_{key}_specific_humidity_IFS_dropsonde_{dt_time:%Y-%m-%d_%H%M}.png')
            plt.close()
