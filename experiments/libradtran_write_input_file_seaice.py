...
The atmosphere is provided by the closest dropsonde measurement from HALO which was converted to the right input format by an IDL script.
...
# %% set paths
    ...
    input_path = f'{libradtran_path}/wkdir/{key}/seaice_{"solar" if solar_flag else "thermal"}'
...
# %% read in INS data
    if use_smart_ins:
        ...
        ins_ds = ins_ds.rename(dict(IRS_LAT='lat', IRS_LON='lon', IRS_ALT='alt'))

# %% read in ifs data
    ifs_ds = (xr.open_dataset(f'{ifs_path}/ifs_{date}_00_ml_O1280_processed_sel_JR.nc')
              .set_index(rgrid=['lat', 'lon']))
    # get IFS along the flight track
    ifs_lat_lon = np.column_stack((ifs_ds.lat, ifs_ds.lon))
...
# %% write input files for each timestep
    ...
    while timestamp < ins_ds.time[-1]:
        ...
        lat, lon, alt = (ins_ds_sel.lat.values,
                         ins_ds_sel.lon.values,
                         ins_ds_sel.alt.values)
        if use_smart_ins:
            # no temperature and pressure available use norm atmosphere
             pres, temp = 1013.25, 288.15
        else:
            pres, temp = ins_ds_sel.PS.values, ins_ds_sel.TS.values

        # select closest column of ifs data
        point = np.deg2rad(np.column_stack([lat, lon]))
        _, idxs = ifs_tree.query(point, k=1)  # query the tree
        ...
        ifs_sel = ifs_ds.sel(rgrid=latlon_sel).sel(time=timestamp, method='nearest')
