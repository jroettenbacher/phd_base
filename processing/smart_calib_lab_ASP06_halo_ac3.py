...
# %% User Input
prop = "VNIR"  # set property to work on (VNIR or SWIR)
...
ax.set_title(f"1000W Lamp Laboratory Calibration\n"
             f"{date_str.replace('_', '-')} {spectrometer} {direction} {prop} {t_int}ms integration time")
...
ax.grid()
plt.tight_layout()
figname = f"{plot_path}/{spectrometer}_{direction}_{prop}_lamp_lab_calib{norm}.png"
...
fig, ax = plt.subplots()
ax.plot(pixel_wl["wavelength"], pixel_wl["F_ulli2"], color="#6699CC", label="Irradiance Ulli2")
ax.plot(pixel_wl["wavelength"], pixel_wl["F_ulli3"], color="#117733", label="Irradiance Ulli3")
ax.set_title(f"Ulli2 and Ulli3 Transfer Sphere Laboratory Calibration \n"
             f"{spectrometer} {direction} {prop} {t_int}ms integration time")
ax.set_xlabel("Wavelength (nm)")
ax.set_ylabel("Irradiance (W$\\,$m$^{-2}$)")
ax2 = ax.twinx()
...
ax.grid()
plt.tight_layout()
figname = f"{plot_path}/{spectrometer}_{direction}_{prop}_ullis_lab_calib{norm}.png"
plt.savefig(figname, dpi=100)
log.info(f"Saved {figname}")
# plt.show()
plt.close()
...
csvname = f"{calib_path}/{spectrometer}_{direction}_{prop}_lab_calib{norm}.dat"
