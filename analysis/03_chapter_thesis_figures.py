...
keys = ["RF17", "RF18"]
ecrad_versions = [f"v{x}" for x in [15.1, 16, 18.1, 19.1, 20, 22, 24, 26, 27, 28]]
...
# %% sea ice - prepare data for box/violin plot
...
# %% sea ice - plot boxplot of below cloud irradiance
_, axs = plt.subplots(1, 2, figsize=h.figsize_wide, layout="constrained")
for i, key in enumerate(keys):
    ax = axs[i]
    df_plot = df[(df.key == key) & (df.label.isin(["BACARDI", "v15.1", "v22", "v18.1", "v24"]))]
    sns.boxplot(df_plot, x="label", y="values", notch=True,
               order=["BACARDI", "v15.1", "v22", "v18.1", "v24"], ax=ax)
...
axs[0].set(ylabel=f"{h.cbarlabels[label]} ({h.plot_units[label]})")
figname = f"03_HALO_AC3_RF17_RF18_{bacardi_var}_BACARDI_ecRad_3d_effects_boxplot.png"
plt.savefig(f"{plot_path}/{figname}", dpi=300)
plt.show()
plt.close()

# %% sea ice - get statistics of boxplot
ci_stats = (df
            .groupby(['key', 'label'])['values']
            .describe()
            .sort_values(["key", "mean"], ascending=[True, False]))

# %% 3D effects - prepare data for box/violin plot
ecrad_var = "transmissivity_sw_above_cloud"
bacardi_var = "transmissivity_above_cloud"
df = pd.DataFrame()
for key in keys:
...
# %% 3D effects - plot violinplot of below cloud transmissivity
_, axs = plt.subplots(1, 2, figsize=h.figsize_wide, layout="constrained")
for i, key in enumerate(keys):
    ax = axs[i]
    df_plot = df[(df.key == key) & (df.label.isin(["BACARDI", "v15.1", "v13", "v13.2"]))]
    sns.violinplot(df_plot, x="label", y="values", notch=True,
                   order=["BACARDI", "v15.1", "v13", "v13.2"], ax=ax)
    ax.set(xlabel="", ylabel="",
           xticklabels=["BACARDI",
                        "ecRad Reference\nsimulation (v15.1)",
                        "ecRad Open ocean\nsimulation (v13)",
...
for i, key in enumerate(keys):
    ax = axs[i]
    df_plot = df[(df.key == key) & (df.label.isin(["BACARDI"] + ecrad_versions))]
    sns.boxplot(df_plot, x="label", y="values", notch=True,
...
# %% 3D effects - get statistics of boxplot
stats = df.groupby(['key', 'label'])['values'].describe()
