...
from bahamas import get_position
from libradtran import find_closest_radiosonde_station

# %% this input will come from the BAHAMAS flight data at one point
flight = "Flight_20210715a"
timestamp = datetime.datetime(year=2021, month=7, day=15, hour=8, minute=45, tzinfo=datetime.timezone.utc)
lat, lon, alt = get_position(flight, timestamp)
zout = alt / 1000  # page 127; aircraft altitude in km
radiosonde_station = find_closest_radiosonde_station(lat, lon)
cos_sza = np.cos(np.deg2rad(get_altitude(lat, lon, timestamp)))
