;cd /projekt_agmwend/data/EUREC4A/10_f_dw_simulations/
;.r Fdw_dirdiff_EUREC4A.pro

;*************************************************************************
;Use this to perform the fdw simulations using the BAHAMAS Navigation Data
;*************************************************************************
...
navfile = file_search(path1+'/BAHAMAS/','*.nas')
...
;read BAHAMAS file
;==============================================================
print,'read BAHAMAS'  & wait,0.1

openr,1,navfile
dummy='' &  readf,1,dummy
dlines=(strsplit(dummy,/extract))(0)

for i=0,dlines-2 do readf,1,dummy

header=strsplit(dummy,/extract)
ilat=where(header eq 'IRS_LAT')
ilon=where(header eq 'IRS_LON')
ialt=where(header eq 'IRS_ALT')

;ilat=where(header eq 'IRS_LAT_NP')  ;for September 3 and 4 only
;ilon=where(header eq 'IRS_LON_NP')
;ialt=where(header eq 'IRS_ALT_NP')

data=fltarr(n_elements(header),file_lines(navfile)-dlines)
readf,1,data
...
;start loop for simulations
 ...
 openw,oo,outfile,/GET_LUN
   ...
   printf,oo,'Direct fraction of downward irradiance along flight track, calculated by libRadtran 2.0.4 using BAHAMAS nav data'
...
endcase
   ...
   printf,oo,'STIPULATIONS_ON_USE: Use of these data requires prior OK from the PI. The ACRIDICON Data Protocol applies.'
 ...
 openw,oo,outfile,/GET_LUN
   ...
   printf,oo,'Wendisch, Manfred' ; PI
   printf,oo,'LIM, Leipzig University' ; Affil
   printf,oo,'Clear sky downward irradiance along flight track, calculated by libRadtran 2.0 using BAHAMAS nav data'
   printf,oo,'Cirrus-HL Flight '+date
   printf,oo,'1 1'  ; only one file
case strmid(systime(/UTC),4,3) of
 'Jan': sysmonth='01'
