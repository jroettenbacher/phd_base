...
# ax.boxplot(F_cor_flat, vert=False, labels=[""], widths=0.9)
...
# F_cor.plot(x="time", robust=True, cmap="inferno",
...
#     F_cor.sel(wavelength=wavelength).plot(y="time", label=f"{wavelength}$\,$nm")
...
# # %% plot roll angle with stabilization status
...
# fig, ax = plt.subplots()
# ds["roll"].plot(x="time", color="#661100")
# stabbi_working.plot(ls="", marker="s", markersize=2, label="working")
# stabbi_not_working.plot(ls="", marker="s", markersize=2, label="not working")
# h.set_xticks_and_xlabels(ax, time_range)
# ax.set_xlabel("Time (UTC)")
# ax.set_ylabel("Roll Angle (°)")
# ax.grid()
# ax.set_ylim([-2, 2.02])
# ax.legend(markerscale=6, title="Stabilization")
# plt.show()
# plt.close()

# %% Plot Fup overview plot - first draft
...
# for wavelength in wavelengths:
#     F_cor.sel(wavelength=wavelength).plot(ax=ax1, y="time", label=f"{wavelength}$\,$nm")
